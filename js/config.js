...
export const Config = {
  ...
  EVENT_ACTIONS: {
    ...
    selectState: {
      ...
      actions: [
        ...
        // The list view is expected to be active already.
      ]
    },
    selectRegion: {
  ...
  LIST_ITEM_TEMPLATES: {
    state: {
      ...
      dataMapping: {
        '[data-list-item="title"]': { type: 'text', source: (p) => p.NAME || 'Unnamed State' },
        '[data-list-item="count"]': { 
            type: 'text', 
            source: (p) => `${p.point_count_abbreviated || p.point_count || 1} beaches` 
        }
      }
    },
    region: {
      ...
      dataMapping: {
        '[data-list-item="title"]': { type: 'text', source: (p) => p['Location Cluster'] || 'Unnamed Region' },
        '[data-list-item="count"]': { 
            type: 'text', 
            source: (p) => `${p.point_count_abbreviated || p.point_count || 1} beaches` 
        }
    ...
    beach: {
      ...
      dataMapping: {
        '[data-list-item="image"]': { type: 'image', source: (p) => p['Main Image'] || 'https://via.placeholder.com/80' },
        '[data-list-item="title"]': { type: 'text', source: (p) => p.Name || 'Beach Title' },
        '[data-list-item="location-cluster"]': { type: 'text', source: (p) => p['Location Cluster'] },
        '[data-list-item="state"]': { type: 'text', source: (p) => p.State },
        '[data-list-item="delimiter"]': { type: 'style', style: { display: 'inline' } } // Ensure delimiter is visible
      }
    }
  }
}; 